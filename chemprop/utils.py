...
def get_loss_func(args: TrainArgs) -> nn.Module:
    ...
    if args.dataset_type == 'regression':
        ...
        if args.confidence == 'evidence':
            return functools.partial(evidential_loss, lam=args.regularizer_coeff)
        ...
        if args.metric == "rmse":
            ...
            raise ValueError(f"metric {args.metric} must be compatible with regression if --data_type=regression")
...
def build_lr_scheduler(optimizer: Optimizer, args: TrainArgs, total_epochs: List[int] = None, scheduler_name : str = "noam") -> _LRScheduler:
    """
